<template>
  <div class="flex flex-col h-full w-full">
    <div
      v-if="!this.hideTop"
      class="bg-quaternary p-2 active:ring-0 active:border-0 flex flex-row items-center justify-between w-full"
    >
      <div />
      <p class="font-bold text-primary text-3xl">Bar Chart</p>
      <button
...
export default {
  ...
  created() {
    ...
    vm.series = null;
    if (Object.getOwnPropertyNames(vm.data[0]).length == 2) {
      vm.chartOptions = {
        xaxis: {
          title: {
            text: vm.features[0],
      ...
      for (let dataPoint of vm.data) {
        if (!vm.chartOptions.xaxis.categories.includes(dataPoint.feature0)) {
          vm.chartOptions.xaxis.categories.push(dataPoint.feature0);
        }
      }
      vm.series = [
        {
      ...
      for (let dataPoint of vm.data) {
        if (dataPoint.feature1) vm.series[0].data.push(dataPoint.feature1);
        else vm.series[0].data.push(dataPoint.feature2);
      }
    }
  },
  methods: {
