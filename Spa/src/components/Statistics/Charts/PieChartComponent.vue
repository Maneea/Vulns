...
export default {
  ...
  created() {
    ...
    vm.series = null;
    if (this.features[0] === "SeverityRange"){
      for(let i = 0; i != vm.data.length; i++){
        let splitted = vm.data[i].feature0.split("-")
        vm.data[i].feature0 = `Severity from ${splitted[0]} to ${splitted[1]}`
      }
    }
    if (Object.getOwnPropertyNames(vm.data[0]).length == 2) {
      ...
      for (let dataPoint of vm.data) {
        vm.chartOptions.labels.push(dataPoint.feature0);
      }
      vm.series = [];
      for (let dataPoint of vm.data) {
        if (dataPoint.feature1)
          vm.series.push(dataPoint.feature1);
        else 
          vm.series.push(dataPoint.feature2);
      }
    }
  },
  methods: {
