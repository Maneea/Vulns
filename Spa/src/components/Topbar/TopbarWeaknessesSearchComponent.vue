<template>
  <vSelect
    ...
    :options="options"
    @search="fetchOptions"
    class="bg-white w-[48rem]"
    :filterBy="filterMethod"
    single
    v-model="selected"
  >
    <template #no-options>
      <div class="italic opacity-75">{{this.options ? "Type to search for a weakness..." : "No results found"}}</div>
    </template>
    ...
    <template #option="{ id, name }">
        <div class="flex flex-col">
          ...
          <p class="italic opacity-50 text-sm">{{ id }}</p>
        </div>
    </template>
  </vSelect>
</template>
...
export default {
  name: "TopbarWeaknessesSearchComponent",
  ...
  watch: { 
    selected(newValue) {
      if(this.selected !== null) {
        this.$emit("update:weakness", this.selected);
        this.selected = null
      }
    }
  },
  props : {
    placeholder: {
      type: String,
      default: "Type to search for a weakness",
      required: false
    }
  },
 created() {
    WeaknessService.getWeaknesses().then(weaknesses => {
